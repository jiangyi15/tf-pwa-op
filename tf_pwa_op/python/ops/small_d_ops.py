...
def delta_D(alpha, beta, gamma, j, la, lb, lc):
    ...
    lc = _cached_li(lc)
    # print(j, la, lb, lc)
    x, y = small_d_ops.DeltaD(
        small_d=d, alpha=alpha, gamma=gamma, la=la, lb=lb, lc=lc, j=j
    )
...
def momentum_lambda(m0, m1, m2):
    ...
    def grad(g):
        return [
            g * momentum_lambda_grad(m0, m1, m2),
...
def get_relative_p2(m0, m1, m2):
    ...
    shape = tf.broadcast_dynamic_shape(
        tf.broadcast_dynamic_shape(m0.shape, m1.shape), m2.shape
    )
