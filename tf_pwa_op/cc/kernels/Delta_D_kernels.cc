        ...
        for (int jb = 0; jb < nb; jb++) {
          for (int jc = 0; jc < nc; jc++) {
            ...
            if (abs(delta) <= j) {
              ...
              T tmp = small_d[idx];
              std::cout << ia << ib << ic << " " << ia / 2 << delta / 2 << " "
                        << idx << std::endl;
              T theta = 0.5 * (ia * alpha[i] + delta * gamma[i]);
              std::cout << theta << std::endl;
              out_r[i * na * nb * nc + ja * nb * nc + jb * nc + jc] =
                  cos(theta) * tmp;
              out_i[i * na * nb * nc + ja * nb * nc + jb * nc + jc] =
                  sin(theta) * tmp;
            }
          }
        }
...
public:
  ...
  void Compute(OpKernelContext *context) override {
    ...
    //                context->allocate_temp(beta.dtype(), shape1,
    //                &sincos_tensor));
    // shape1.AddDim(j_ + 1);
    // OP_REQUIRES_OK(context,
    //                context->allocate_temp(beta.dtype(), shape1,
    ...
    DeltaDFunctor<Device, T>()(
        context->eigen_device<Device>(), static_cast<int>(alpha.NumElements()),
        j_, small_d_tensor.flat<T>().data(), alpha.flat<T>().data(),
        gamma.flat<T>().data(), la.flat<int32>().data(),
        lb.flat<int32>().data(), lc.flat<int32>().data(),
        output_r_tensor->flat<T>().data(), output_i_tensor->flat<T>().data(),
        na, nb, nc);
  }
  int j_;
};
